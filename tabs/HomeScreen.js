  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.big}>{total === 0 ? '0%' : `${Math.round((done / total) * 100)}%`}</Text>
        <Text style={styles.meta}>{done} completed • {Math.max(total - done, 0)} remaining</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { marginTop: 18, backgroundColor: colors.surface, borderRadius: 18, padding: 16, borderWidth: 1, borderColor: colors.border },
